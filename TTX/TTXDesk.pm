...
$TTXDesk::VERSION='3.01';
BEGIN {
  $TTXDesk::REVISION = '$Revision: 769 $';
  ...
  foreach my $fld (@fltrcols) {
    ...
    if ($fld eq 'oper') {
      ...
      if ($1 ne undef && $cfg->get("dropdown$1") ne undef) {
        ...
        foreach my $option (@options) {
          next if $option eq undef && $val eq undef;
          my $selected = " selected" if $option eq $val;
          my $v = $option;
          $v =~ s/"/&quot;/g;
          my $h = $option;
...
sub helpdesk {
  ...
  $c2T{item} = TTXCommon::decodeit($cfg->get('inventory.label')) || 'Item';
  $c2t{item} = lc TTXCommon::decodeit($cfg->get('inventory.label')) || 'item';
  my $cfldcnt = TTXCommon::cfldcnt();
  ...
  my $checkgrp = 0;
  my @memberof;
  if ($usegroups && $cfg->get('hideoperators')) {
    $checkgrp = 1;
    @memberof = TTXGroups::grouplist($cfg, $userid);
  }
  ...
  foreach my $id (@grplist) {
    next if $checkgrp && !grep(/^$id$/, @memberof);
  ...
  my @olist = @operlist;
  if ($checkgrp) {
    @olist = TTXGroups::cleanoperlist($cfg, \@operlist);
  }
  foreach my $oper (sort @olist) {
  ...
  foreach my $status ('(OPN|PND)', 'PND', 'OPN', 'WFR', 'HLD', '(OPN|WFR)', 'CLS', '(PND|OPN|WFR)') {
  ...
  if (@{$list} < 1) {
    ...
    my $i = 0;
    my $j = 0;
    $rclr[0] = $cfg->get('color-helpdesk-table-erow');
    $rclr[1] = $cfg->get('color-helpdesk-table-orow');
    my %colormatrix = (
      PND => $cfg->get('color-helpdesk-table-pending'),
      WFR => $cfg->get('color-helpdesk-table-responded'),
      OPN => $cfg->get('color-helpdesk-table-open'),
      CLS => $cfg->get('color-helpdesk-table-solved'),
      HLD => $cfg->get('color-helpdesk-table-onhold'),
      LATE => $cfg->get('color-helpdesk-table-late') || 'red'
    );
    my $now = time();
    foreach my $id (@{$list}) {
      my ($fcopen, $fcclose);
      ...
      if ($query->param('cmd') ne 'mytickets' && !$readonly) {
        $data->{TICKETLIST} .= "<td align=center valign=middle class=$cl><input type=checkbox name=mtid value=".$t->{id}."></td>\n";
      }
      foreach my $col (@brcols) {
        my $val = $t->{$col};
        my $align = 'right';
        if (grep(/^$col$/, ('open', 'updated', 'closed'))) {
		...
		} elsif ($col eq 'c'.$cfg->get('firstanswer')) {
		  if ($cfg->get('firstanswer.unix')) {
			$val = TTXCommon::tmtxt($val);
		  }
          $val = "<nobr>$fcopen$val$fcclose</nobr>";
        } elsif ($col eq 'status') {
          ...
          $val = $fcopen.(TTXCommon::decodeit($val)).$fcclose;
      ...
      if ($browsetxt) {
        ...
        if ($browsetxt < 2) {
          my $midx = 0;
          if ($cfg->get('abstract.last') && $user ne undef) {
            $midx = int(@{$t->{messages}}) - 1;
            $midx = 0 if $midx < 0;
          }
          my $msg = $t->{messages}->[$midx]; # shift (@{$t->{messages}});
          my @msgparts = split(/\n\n/, $msg);
          shift @msgparts;
          $body = join("\n\n", @msgparts);
          $body =~ s/</&lt;/g;
  ...
  if ($session ne undef) {
    my @savelist = ('text', 'oper', 'tid', 'status', 'lname', 'name', 'email', 'fltrsubj', 'grp',
                    ...
                    'drfld', 'drsday', 'drsmon', 'drsyear', 'dreday', 'dremon', 'dreyear', 'autorefresh', 'item');
    for (my $i = 0; $i < $cfldcnt; ++$i) {
      push @savelist, "c$i";
    }
