...
#
# Fix for Microsoft Entourage 2008 bug
#
if ($query->param('cmd') =~ /=/) {
  my @parts = split(/=/, $query->param('cmd'));
  $query->param('cmd', $parts[0]);
  if ($query->param('key') eq undef) {
    $query->param('key', $parts[1]);
  }
}
#
# XSS cleanup
#
foreach my $id ('key', 'style', 'sid', 'isticket', 'id', 'nextcmd', 'cmd', 'tk', 'emailkey', 'qsort',
                'qsortorder', 'qoffset', 'qwindow', 'autorefresh', 'tid', 'form', 'email', 'ccmail') {
  my $var = TTXCommon::cleanit($query, $id);
  if (length($var)) {
    $var =~ s/</&lt;/g;
    $var =~ s/>/&gt;/g;
    $var =~ s/"/&quot;/g;
...
if ($query->param('checkupdate')) {
  $data{CHECKUPDATE} = "<a href=\"http://www.troubleticketexpress.com\"><img border=0 src=\"http://www.troubleticketexpress.com/cgi-bin/ttx/checkupdate1.cgi?$version\"></a>";
}
...
if ($data{ERROR_MESSAGE} ne undef) {
  ...
  my $bgcolor = $cfg->get("color-$cmd-background") || "#CFDCE8";
  $data{ERROR_BOX} =<<EOT;
...
# Cleanup
#
if (TTXData::get('SMTPOBJ') ne undef) {
  eval 'TTXMail::quit()'; # fool proof
}
exit;
...
sub loadtemplate {
  ...
  if ($t ne '#') {
    ...
    if ($style ne undef) {
      if ((! -f "$fn/$style/$t" || !open(TMPL, "$fn/$style/$t")) &&
        ...
        fatalerror("Missing template $qt");
      }
  ...
  if (!($cmddef->{$query->param('cmd')}->{noheader} || $query->param('noheader'))) {
    if (($style ne undef && (-f "$fn/$style/header.shtml") && open(TMPL, "$fn/$style/header.shtml")) || open(TMPL, "$fn/header.shtml")) {
      my @hdr = <TMPL>;
      close TMPL;
      $template = join('', @hdr) . $template;
    }
    if (($style ne undef && (-f "$fn/$style/footer.shtml") && open(TMPL, "$fn/$style/footer.shtml")) || open(TMPL, "$fn/footer.shtml")) {
      my @ftr = <TMPL>;
      close TMPL;
      $template .= join('', @ftr);
    }
...
sub command {
  ...
  if (!defined $cmddef->{$cmd}) {
    ...
    $qcmd =~ s/</&lt;/g;
    $qcmd =~ s/>/&gt;/g;
    fatalerror("Unknown command [$qcmd]");
  }
