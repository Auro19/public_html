...
$TTXCommon::VERSION='3.01';
BEGIN {
  $TTXCommon::REVISION = '$Revision: 769 $';
...
sub tickedvars {
  ...
  if ($cfg->get('usegrpsel')) {
    $data->{TICKEDHEIGHT} += 20;
  }
  if ($cfg->get('multigroup')) {
    $data->{TICKEDHEIGHT} += 80;
  }
...
sub splitlongwords {
  ...
  if (!$html) {
    ...
    my @strings = split(/<[^<>]*>/, $txt);
    my @parts;
    foreach my $string (@strings) {
      $string =~ s/($tokens{60})/$1\n/g;
      push @parts, $string;
      push @parts, shift @tags if @tags > 0;
    }
    $txt = join('', @parts); 
  }
  return $txt;
