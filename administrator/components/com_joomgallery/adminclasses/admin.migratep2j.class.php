<?php
// $HeadURL: http://joomlacode.org/svn/joomgallery/JG-1.0/JG/trunk/administrator/components/com_joomgallery/adminclasses/admin.migratep2j.class.php $
// $Id: admin.migratep2j.class.php 396 2009-03-15 16:06:21Z aha $
/******************************************************************************\
**   JoomGallery  1.0.1                                                       **
...
class Joom_Migrate_P2J {
  ...
  function Joom_Migrate_P2J($action) {
    global $mosConfig_absolute_path,$mosConfig_dbprefix,$mosConfig_offline;
    ...
    $abs_path=$mosConfig_absolute_path;

    $this->prefix = $mosConfig_dbprefix;
    $this->sitestatus = $mosConfig_offline;
    ...
    $joomconfig = $abs_path . $this->DS
                  ...
                  . "com_joomgallery"
                  . $this->DS
                  . "joomgallery.config.php";

    if (file_exists($ponyconfig)) {
      include_once ($ponyconfig);                   
      $this->galvars['pgimgpic'] = $abs_path . $ag_pathimages;
      $this->galvars['pgorig'] = $abs_path . $ag_pathoriginalimages;
      $this->galvars['pgthumb'] = $abs_path . $ag_paththumbs;
    } else {
      $this->galvars['pgimgpic'] = _JGA_PONYGALLERY . _JGA_PICTURES_DIRECTORY;
      $this->galvars['pgorig']   = _JGA_PONYGALLERY . _JGA_ORIGINALS_DIRECTORY;
      $this->galvars['pgthumb']  = _JGA_PONYGALLERY . _JGA_THUMBNAILS_DIRECTORY;
    }

    if (file_exists($joomconfig)) {
      include_once ($joomconfig);
      $this->galvars['jgimgpic'] = $abs_path . $jg_pathimages;
      $this->galvars['jgorig'] = $abs_path . $jg_pathoriginalimages;
      $this->galvars['jgthumb'] = $abs_path . $jg_paththumbs;
    } else {
      $this->galvars['jgimgpic'] = _JGA_JOOMGALLERY . _JGA_PICTURES_DIRECTORY;
      $this->galvars['jgorig']   = _JGA_JOOMGALLERY . _JGA_ORIGINALS_DIRECTORY;
      $this->galvars['jgthumb']  = _JGA_JOOMGALLERY . _JGA_THUMBNAILS_DIRECTORY;
    }
  ...
  */
  function Joom_Migrate_Check() {
    ...
    $info .= "\n"; 
    $info .= "    <td colspan=\"3\">\n"; 
    $info .= "      <h4>"._JGA_SITESTATUS."</h4>\n"; 
    $info .= "    </td>\n"; 
    $info .= "  </tr>\n"; 
    $info .= "  <tr>\n"; 
    $info .= "    <td width=\"80%\" align=\"left\">\n"; 
    $info .= "      "._JGA_SITE_OFFLINE."\n"; 
    $info .= "    </td>\n"; 
    ...
    $info .= "  <tr>\n"; 
    $info .= "    <td colspan=\"3\">\n"; 
    $info .= "      <h4>"._JGA_CHECK_DIRECTORIES."</h4>\n"; 
    $info .= "    </td>\n"; 
     $info .= "  </tr>\n"; 
    ...
    $info .= "  <tr>\n"; 
    $info .= "    <td colspan=\"3\">\n"; 
    $info .= "      <h4>"._JGA_CHECK_DATABASETABLES."</h4>\n"; 
    $info .= "    </td>\n"; 
    $info .= "  </tr>\n"; 
    ...
    foreach ($this->joomdb as $jdb) {
      ...
      if ($ct['result'] == TRUE && $ctcount == 0 ) {
        ...
        $info .= "  <tr>\n"; 
        $info .= "    <td align=\"left\">\n"; 
        $info .= "      ".$jdb.": <b><span style=\"color:#080; font-size:12px; font-weight:bold;\">"._JGA_EMPTY . "</span></b>\n"; 
        $info .= "    </td>\n"; 
        $info .= "    <td align=\"center\">\n"; 
        $info .= "      <img src=\"images/$img\" border=\"0\" alt=\"\" />\n"; 
        $info .= "    </td>\n"; 
        ...
        $info .= "  <tr>\n"; 
        $info .= "    <td align=\"left\">\n"; 
        $info .= "      ".$jdb.": <b><span style=\"color:#f30; font-weight:bold;\">".$ctcount ." "._JGA_ROWS . "</span></b>\n"; 
        $info .= "    </td>\n"; 
        $info .= "    <td align=\"center\">\n"; 
        $info .= "      &nbsp;\n"; 
        $info .= "    </td>\n"; 
    ...
    foreach ($this->ponydb as $pdb) {
      ...
      if ($ct['result'] == TRUE) {
        ...
        if ($ct['result'] == TRUE && $count == 0 ) {
          ...
          $info .= "  <tr>\n"; 
          $info .= "    <td align=\"left\">\n"; 
          $info .= "      ".$pdb.": <b><span style=\"color:#080; font-size:12px; font-weight:bold;\">"._JGA_EMPTY . "</span></b>\n"; 
          $info .= "    </td>\n"; 
          $info .= "    <td align=\"center\">\n"; 
          $info .= "      <img src=\"images/$img\" border=\"0\" alt=\"\" />\n"; 
          $info .= "    </td>\n"; 
          ...
          $info .= "  <tr>\n"; 
          $info .= "    <td align=\"left\">\n"; 
          $info .= "      ".$pdb.": <b><span style=\"color:#080; font-weight:bold;\">".$count ." "._JGA_ROWS . "</span></b>\n"; 
          $info .= "    </td>\n"; 
          $info .= "    <td align=\"center\">\n"; 
          $info .= "      <img src=\"images/$img\" border=\"0\" alt=\"\" />\n"; 
          $info .= "    </td>\n"; 
    ...
    if ($resultarr['result'] != FALSE) {   
      ...
      if ($ctcount == 0 ) {
        ...
        $info .= "  <tr>\n"; 
        $info .= "    <td align=\"left\">\n"; 
        $info .= "      "._JGA_MIGRATION_ORPHANPICS."<b><span style=\"color:#080; font-size:12px; font-weight:bold;\">"._JGA_NO."</span></b>\n"; 
        $info .= "    </td>\n"; 
        $info .= "    <td align=\"center\">\n"; 
        $info .= "      <img src=\"images/$img\" border=\"0\" alt=\"\" />\n"; 
        $info .= "    </td>\n"; 
        ...
        $info .= "  <tr>\n"; 
        $info .= "    <td align=\"left\">\n"; 
        $info .= "      "._JGA_MIGRATION_ORPHANPICS."<b><span style=\"color:#f30; font-weight:bold;\">".$ctcount."</span></b>\n"; 
        $info .= "    </td>\n"; 
        $info .= "    <td align=\"center\">\n"; 
        $info .= "      &nbsp;"; 
        $info .= "    </td>\n"; 
    ...
    if (!$ready) {
      $this->Joom_Migrate_HTML(false,$info,
                              ...
                              '. _JGA_MIGRATION_FALSE.'</div><div style="text-align:center;">'._JGA_MIGRATION_FALSE_LONG.'</div>');
    } else {
      $this->Joom_Migrate_HTML(true,$info,
                               '<div style="text-align:center;color:#080;padding:1em 0;font:bold 1.2em Verdana;">
                               '. _JGA_MIGRATION_TRUE.'</div><div style="text-align:center;">'._JGA_MIGRATION_TRUE_LONG.'</div>');
    }
    return true;
  }
  ...
  function Joom_Migrate_CreateJoomCats() {
    ...
    foreach ($arrayref as $catdir) {
      $this->Joom_Migrate_DirStructure($this->galvars['jgimgpic'] . $this->DS . $catdir['catpath']);
      touch($this->galvars['jgimgpic'] . $this->DS . $catdir['catpath'] . '/index.html');

      $this->Joom_Migrate_DirStructure($this->galvars['jgorig'] . $this->DS . $catdir['catpath']);
      touch($this->galvars['jgorig'] . $this->DS . $catdir['catpath'] . '/index.html');

      $this->Joom_Migrate_DirStructure($this->galvars['jgthumb'] . $this->DS . $catdir['catpath']);
      touch($this->galvars['jgthumb'] . $this->DS . $catdir['catpath'] . '/index.html');
    }

    //fill joomgallery_catgs in a db transaction
    ...
    //check the time, if true, we starts with migration of the pictures directly
    //without refresh
    if ($this->Joom_Migrate_Checktime() == true) {
      $this->Joom_Migrate_WriteLogfile ("Starting with migration of pictures directly");      
      $this->Joom_Migrate_Move();  
    } else {
      //Make an refresh to start new with moving the pictures
   ...
   *
   */
  function Joom_Migrate_DirStructure($dir, $mode = 0755) {
    if (!is_dir($dir)) {
      $this->Joom_Migrate_DirStructure(dirname($dir), $mode);
      mkdir($dir, $mode);
    }
  }
  ...
  function Joom_Migrate_Rename($sourcefilename, $sourcethumbname, $destcatpath) {
    //Originals
    if (file_exists($this->galvars['pgorig'] . $this->DS . $sourcefilename)) {
      $result = rename($this->galvars['pgorig'] . $this->DS . $sourcefilename, $this->galvars['jgorig'] . $this->DS .$destcatpath . $this->DS . $sourcefilename);
      if (!$result) {
        var_dump($result);
        return false;
      }
    ...
    //Details
    $result = rename($this->galvars['pgimgpic'] . $this->DS . $sourcefilename, $this->galvars['jgimgpic'] . $this->DS .$destcatpath . $this->DS . $sourcefilename);
    if (!$result) {
      var_dump($result);
      return false;
    }
    //Thumbs
    $result = rename($this->galvars['pgthumb'] . $this->DS . $sourcethumbname, $this->galvars['jgthumb'] . $this->DS .$destcatpath . $this->DS . $sourcethumbname);
    if (!$result) {
      var_dump($result);
      return false;
    }
  ...
  function Joom_Migrate_Refresh($action) {
    if ($action == 'continue') {
      $scriptname = "index2.php?option=com_joomgallery&act=migrate&migration=p2jcontinue";
      $this->Joom_Migrate_WriteLogfile ("Refresh to continue the migration");
    } else {
      $scriptname = "index2.php?option=com_joomgallery&mosmsg="."Migration successfully ended";
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
  ...
  <tr>
    ...
    <td colspan="3"><hr /><?php echo $result; ?>
    <hr />
    </td>
  </tr>
  <tr>
...
?>
    ...
    <form action="index2.php?option=com_joomgallery&amp;act=migrate" method="POST">
      <input type="submit" name="migratep2j" value="start" style="width: 100px" />
    </form>
    <hr />
    </th>
  <?php
