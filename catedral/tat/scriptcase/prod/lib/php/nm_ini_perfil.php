...
function nm_prod_load_lang($dir)
{
    ...
    if (!isset($_SESSION['nm_session']['prod_v7']['lang']) ||
        !@is_file($dir . '/lang.' . $_SESSION['nm_session']['prod_v7']['lang'] . '.php'))
    {
        $arr_accept = array();
        $str_accept = '';
    ...
    include($dir . '/lang.' . $_SESSION['nm_session']['prod_v7']['lang'] . '.php');
} // nm_prod_load_lang

function carrega_perfil($perfil, $dir, $prot = 'S', $dir_prod_conf="")
{
    if (isset($_SESSION['nm_session']['prod_v7'][$perfil]))
    {
        foreach ($_SESSION['nm_session']['prod_v7'][$perfil] as $var => $dados)
        {
    ...
    if (isset($prod_ini_xml["PROFILE"]) && is_array($prod_ini_xml["PROFILE"]) && in_array($perfil, array_keys($prod_ini_xml["PROFILE"])))
    {
        if ("Y" == $prod_ini_xml["PROFILE"][$perfil]["USE_HOST"])
        {
            ...
            $_SESSION['nm_session']['prod_v7'][$perfil]['glo_servidor'] = $glo_servidor;
        }
        if ("Y" == $prod_ini_xml["PROFILE"][$perfil]["USE_USER"])
        {
            $glo_usuario = decode_string($prod_ini_xml["PROFILE"][$perfil]["VAL_USER"]);
            $_SESSION['nm_session']['prod_v7'][$perfil]['glo_usuario'] = $glo_usuario;
        }
        if ("Y" == $prod_ini_xml["PROFILE"][$perfil]["USE_PASS"])
        {
            if (isset($prot) && 'N' == $prot)
            {
                ...
                $_SESSION['nm_session']['prod_v7'][$perfil]['glo_senha_protect'] = "N";
            }
            else
            {
                $glo_senha         = $prod_ini_xml["PROFILE"][$perfil]["VAL_PASS"];
                $_SESSION['nm_session']['prod_v7'][$perfil]['glo_senha_protect'] = "S";
            }
        }
        else
        {
        ...
        if ("" != decode_string($prod_ini_xml["PROFILE"][$perfil]["POSTGRES_ENCODING"]))
        {
                ...
                $_SESSION['nm_session']['prod_v7'][$perfil]['glo_database_encoding'] = $glo_database_encoding;
        }
        //oracle
        ...
        if ("" != decode_string($prod_ini_xml["PROFILE"][$perfil]["DB2_AUTOCOMMIT"]))
        {
                ...
                $_SESSION['nm_session']['prod_v7'][$perfil]['glo_db2_autocommit'] = $glo_db2_autocommit;
        }
        if ("" != decode_string($prod_ini_xml["PROFILE"][$perfil]["DB2_I5_LIB"]))
        {
                ...
                $_SESSION['nm_session']['prod_v7'][$perfil]['glo_db2_i5_lib'] = $glo_db2_i5_lib;
        }
        if ("" != decode_string($prod_ini_xml["PROFILE"][$perfil]["DB2_I5_NAMING"]))
        {
                ...
                $_SESSION['nm_session']['prod_v7'][$perfil]['glo_db2_i5_naming'] = $glo_db2_i5_naming;
        }
        if ("" != decode_string($prod_ini_xml["PROFILE"][$perfil]["DB2_I5_COMMIT"]))
        {
                ...
                $_SESSION['nm_session']['prod_v7'][$perfil]['glo_db2_i5_commit'] = $glo_db2_i5_commit;
        }
        if ("" != decode_string($prod_ini_xml["PROFILE"][$perfil]["DB2_I5_QUERY_OPTIMIZE"]))
        {
                ...
                $_SESSION['nm_session']['prod_v7'][$perfil]['glo_db2_i5_query_optimize'] = $glo_db2_i5_query_optimize;
        }
        //fimbanco db2
...
function db_conect_devel($str_conn, $str_path, $projeto, $opcao = 1)
{
    ...
    if ('NetMake' != $str_conn)
    {
        ...
        $conex  = $db_sys->Execute("select Sgdb, Servidor, Usuario, Senha, Banco, Simb_Decimal, Attr1
                                     from ". $esquema ."sc_tbconex where Cod_Prj = '$projeto' and Nome = '$str_conn'");
        if (!$conex->EOF)
        {
