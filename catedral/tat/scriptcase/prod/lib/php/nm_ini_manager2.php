...
if (!isset($_SESSION['nm_session']['prod_v7']))
{
    $_SESSION['nm_session']['prod_v7'] = array('logged' => FALSE);
}

if(isset($_GET['login']) && isset($_SESSION['nm_session']['cache']['prod_v7']))
{
	unset($_SESSION['nm_session']['cache']['prod_v7']);
	
	if(isset($_SESSION['nm_session']['prod_v7']['logged']))
	{
		unset($_SESSION['nm_session']['prod_v7']['logged']);
	}
}
...
$str_option = '';
nm_prod_lang();
include $nm_config['path_lib'] . 'lang.' . $_SESSION['nm_session']['prod_v7']['lang'] . '.php';

// Arquivo de inicializacao
if (!defined('NM_INC_PROD_INI'))
{
    include_once $nm_config['path_lib'] . 'nm_ini_lib.php';
    include_once $nm_config['path_lib'] . 'nm_serialize.php';
}
$str_ini = $nm_config['path_conf'] . 'prod.config.php';
$arr_ini = nm_unserialize_ini($str_ini);

$_SESSION['nm_session']['prod_v7']['arr_dbms'] = $arr_dbms;
...
if (('' != $str_option) && ('login' != $str_option) && !$_SESSION['nm_session']['prod_v7']['logged'])
{
    $str_option  = '';
    $str_opt_par = '';
}
...
switch ($str_option)
{
    ...
    case 'change_pass':
        if (isset($arr_argv['field_pass_old']))
        {
            ...
            {
				$arr_ini['GLOBAL']['LANGUAGE'] = $_SESSION['nm_session']['prod_v7']['lang'];
    ...
    case 'environment':
        if (isset($arr_argv['field_gc_dir']))
        {
            ...
            if (empty($arr_error))
            {
                $_SESSION['nm_session']['prod_v7']['lang'] = $arr_argv['field_language'];
                include $nm_config['path_lib'] . 'lang.' . $_SESSION['nm_session']['prod_v7']['lang'] . '.php';
                $str_xml = nm_serialize_ini($arr_ini);
				salva_xml($str_ini, $str_xml, $arr_ini);
    ...
    // Realiza login
    case 'login':
    	if ($_SESSION['nm_session']['prod_v7']['lang'] != $arr_argv['field_language'])
    	{
            if (!@is_file($nm_config['path_lib'] . 'lang.' . $arr_argv['field_language'] . '.php'))
            {
                $arr_error[] = 'error_lang_file_invalid';
    	...
    	else
    	{
	        ...
	        elseif ('scriptcase' == $str_pwd_htm)
	        {
	            ...
	            $_SESSION['nm_session']['prod_v7']['logged'] = TRUE;
	        }
	        else
	        {
	            $str_option = 'main_menu';
    ...
    // Menu principal
    case 'logout':
        $_SESSION['nm_session']['prod_v7']['logged'] = FALSE;
        $str_option = 'login';
    break;
    // Menu principal
    case 'main_menu':
...
function nm_prod_display_environment()
{
    ...
    $str_comp = ('' == $arr_ini['GLOBAL']['LANGUAGE']) ? $_SESSION['nm_session']['prod_v7']['lang'] : $arr_ini['GLOBAL']['LANGUAGE'];
?>
...
<table class="nmTable" cellpadding="3px">
 ...
 <tr>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  <td class="nmLineV3"><?php echo $nm_lang['form_gc_dir']; ?></td>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  <td class="nmLineV3"><input type="text" name="field_gc_dir" size="50" value="<?php echo $arr_ini['GLOBAL']['GC_DIR']; ?>" class="nmInput" /></td>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
 </tr>
 <tr>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  <td class="nmLineV3"><?php echo $nm_lang['form_gc_min']; ?></td>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  <td class="nmLineV3"><input type="text" name="field_gc_min" size="5" value="<?php echo $arr_ini['GLOBAL']['GC_MIN']; ?>" class="nmInput" /></td>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
 </tr>
 <tr>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  <td class="nmLineV3"><?php echo $nm_lang['form_language']; ?></td>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  <td class="nmLineV3">
...
?>
 ...
 <tr>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  <td class="nmLineV3"><?php echo $nm_lang['form_googlemaps_api_key']; ?></td>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  <td class="nmLineV3"><input type="text" name="field_googlemaps_api_key" size="25" value="<?php echo (isset($arr_ini['GLOBAL']['GOOGLEMAPS_API_KEY']) ? $arr_ini['GLOBAL']['GOOGLEMAPS_API_KEY'] : ""); ?>" class="nmInput" /></td>
  <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
 </tr>
 <tr><td colspan="5" style="font-size:3px">&nbsp;</td></tr>
</table>
...
<input type="button" value="<?php echo $nm_lang['button_save']; ?>" onclick="nm_set_option('environment', '', 'nm_iframe')" class="nmButton" >&nbsp;&nbsp;&nbsp;
<input type="button" value="<?php echo $nm_lang['button_cancel']; ?>" onClick="nm_set_option('main_menu', '', '_top')" class="nmButton" />
</center>
</div>
...
<head>
 ...
 <meta http-equiv="Content-Type" content="text/html; charset=<?php echo $nm_lang['charset']; ?>" />
...
function nm_prod_display_login($v_str_pass, $v_tp_win = "login")
{
    ...
    $arr_lang = nm_prod_list_lang();
    $str_comp = ('' == $_SESSION['nm_session']['prod_v7']['lang']) ? $arr_ini['GLOBAL']['LANGUAGE'] : $_SESSION['nm_session']['prod_v7']['lang'];
?>
	<style>
...
<table cellpadding=0 cellspacing=0 border=0 id="loginContainer" align='center'>
    <tr>
        <td style='height: 89%'>
			<!-- logo -->
			<div id="loginPartsLogo"><img src="../../img/logoLogin.png" width="346" height="85" alt="Scriptcase" title="Scriptcase" /></div>
			<!-- xx logo -->
					<div id="loginPartsBox">
						...
						<div id="loginBoxForm">
							...
							<table cellpadding="8" cellspacing="4" border="0">
								...
								?>
								<tr>
									<td valign="top" height="35px" class="nmText" align="left">
								...
								?>
								<tr>
									<td valign="middle" class="nmText" align="right">
					...
					<div id="loginPartsInfos">
						...
						<div id="id_login_msg">
							...
							{
								echo "<span class='nmErrorMsg'>" . $nm_lang['login_warning'] . "</span>";
							}
							?>
						</div>
					</div>
		...
		<td width="300" valign="top" style="padding:10px" class="nmTable">
			...
			<table class="nmTable" cellpadding="0" cellspacing="0" width="100%">
			 ...
			 <tr class="nmTableTd">
			  <td class="nmLineV3">
			  <?php
				switch($_SESSION['nm_session']['prod_v7']['lang'])
				{
...
<table class="nmTable" style="width: 500px" align="center">
 ...
 <tr>
  ...
  <td class="nmLineV3">
<?php
    if (1 == sizeof($arr_dbms[$_SESSION['nm_session']['prod_v7']['profile']['dbms_group']]))
    {
        reset($arr_dbms[$_SESSION['nm_session']['prod_v7']['profile']['dbms_group']]);
        $str_dbms = current($arr_dbms[$_SESSION['nm_session']['prod_v7']['profile']['dbms_group']]);
?>
...
<?php
        foreach ($arr_dbms[$_SESSION['nm_session']['prod_v7']['profile']['dbms_group']] as $str_dbms)
        {
?>
    <option value="<?php echo $str_dbms; ?>"<?php if ($str_dbms == $_SESSION['nm_session']['prod_v7']['profile']['dbms']) { echo ' selected="selected"'; } ?>><?php echo $nm_lang[$str_dbms]; ?></option>
<?php
        }
?>
   </select>
 ...
 <tr>
  ...
  <td class="nmLineV3">
   <select name="field_decimal" class="nmInput">
    <option value="."<?php  if ('.' == $_SESSION['nm_session']['prod_v7']['profile']['decimal']) { echo ' selected="selected"'; } ?>><?php echo $nm_lang['form_dec_dot'];   ?></option>
    <option value=","<?php  if (',' == $_SESSION['nm_session']['prod_v7']['profile']['decimal']) { echo ' selected="selected"'; } ?>><?php echo $nm_lang['form_dec_comma']; ?></option>
   </select>
  </td>
 </tr>
 <tr>
  ...
  <td class="nmLineV3">
   <select name="field_use_persistent" class="nmInput">
    <option value="Y"<?php  if ('Y' == $_SESSION['nm_session']['prod_v7']['profile']['use_persistent']) { echo ' selected="selected"'; } ?>><?php echo $nm_lang['form_persistente_y'];   ?></option>
    <option value="N"<?php  if ('N' == $_SESSION['nm_session']['prod_v7']['profile']['use_persistent'] || empty($_SESSION['nm_session']['prod_v7']['profile']['use_persistent'])) { echo ' selected="selected"'; } ?>><?php echo $nm_lang['form_persistente_n']; ?></option>
   </select>
  </td>
 </tr>
</table>
...
{
    global $nm_config;
    if (!isset($_SESSION['nm_session']['prod_v7']['lang']) ||
        !@is_file($nm_config['path_lib'] . 'lang.' . $_SESSION['nm_session']['prod_v7']['lang'] . '.php'))
    {
        $arr_accept = array();
        $str_accept = '';
        ...
        $_SESSION['nm_session']['prod_v7']['lang'] = $str_accept;
    }
} // nm_prod_lang
...
function nm_prod_path()
{
    ...
    if(!empty($dir_getcwd))
    {
        ...
        $str_path_base = str_replace("//", "/", str_replace("\\", "/", $dir_getcwd)) . "/" . $arquivo[count($arquivo)-1];
    }else
    {
        ...
        $str_path_base = str_replace("//", "/", str_replace("\\", "/", $str_path_base));
    }
    //fim alteracao diogo
