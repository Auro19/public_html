...
class nmPageAdminSysAllConectionsCreateWizard extends nmPage
{
     ...
     *
     * Template que será exibido no form
     *
     * @access  protected
     * @var     array
     */
    ...
    /**
     * Retorna os passo de criaçao
     *
     * Rotorna o array com os passos de criação
     *
     * @access  protected
     */
    function GetSteps()
   ...
   /**
    ...
    {
		$arr_ini = $_SESSION['nm_session']['prod_v7']['arr_ini'];
  ...
  function GetNextStep()
  {
    ...
    function PageJavascript()
    {
        ...
        $str_js .= "{ return; \n";
        $str_js .= "	if (document.form_create.dbms.value == 'mysql') \n";
        $str_js .= "	{  \n";
        ...
        $str_js .= "            type: 'POST',\n";
        $str_js .= "            url: '" . $nm_config['url_iface'] . 'admin_sys_allconections_create_wizard.php' ."',\n";
        $str_js .= "            data: 'ajax=S&list_db=S&exit=S&host=' + escape(host) + '&usr=' + escape(usr) + '&pwd=' + escape(pwd) + '&db=' + escape(db)+ '&first_time=' + first_time,\n";
        $str_js .= "            success: function(html_retorno)\n";
        $str_js .= "            {\n";
   ...
   /**
    ...
    function ValidateField($v_str_field)
    {
        ...
        switch ($v_str_field)
        {
          ...
          case 'conn':
          	...
          	{
	            if(!$nm_validator->IsCode($mix_val, 1, 128))
	            {
	              $str_err = $nm_lang['create_conn_wizard']['erro']['conn'];
	            }
	            else
	            {
	...
	{
		return isset($_SESSION['nm_session']['prod_v7']['arr_ini']['PROFILE']) &&  isset($_SESSION['nm_session']['prod_v7']['arr_ini']['PROFILE'][$str_conn]);
	}
    ...
    function Ajax()
    {
    	if (isset($_POST['ajax']) && $_POST['ajax'] == 'S')
    	{
    		if (isset($_POST['list_db']) && $_POST['list_db'] == 'S')
    		{
    			...
    			$list_db = $this->GetListDatabaseNameMySql($_POST['host'], $_POST['usr'], $_POST['pwd']);

    			if (is_array($list_db))
    			{
					if (!empty($list_db) || $_POST['first_time'] == 'S')
					{
						$ret = "<select name='sel_base' id='sel_base' class='nmInput' onchange=\"$('#base').val(this.value);\"  onfocus='fc_get_db();'>";
	...
	function AjustePort()
	{
		if ($this->GetArg('dbms') == 'mysql' && $this->IsArg('port') && $this->GetArg('port') != '' && $this->GetArg('server') != '')
		{
	...
	function GetAllConnections()
	{
		...
		foreach ($_SESSION['nm_session']['prod_v7']['arr_ini']['PROFILE'] as $str_conn => $arr_conn)
		{
	...
	function CheckDelConn()
	{
		...
		if ($this->IsArg('del_conn') && $this->GetArg('del_conn') != '')
		{
			...
			foreach ($_SESSION['nm_session']['prod_v7']['arr_ini']['PROFILE'] as $str_conn => $arr_conn)
			{
				...
				{
					unset($_SESSION['nm_session']['prod_v7']['arr_ini']['PROFILE'][$str_conn]);
					break;
				}
			}

			$this->obj_conn->SaveConn($_SESSION['nm_session']['prod_v7']['arr_ini']);
	...
	{
		$arr_ini_conn = $_SESSION['nm_session']['prod_v7']['arr_ini']['PROFILE'][$this->GetArg('conn')];
