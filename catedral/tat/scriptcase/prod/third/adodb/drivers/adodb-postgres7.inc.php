...
class ADODB_postgres7 extends ADODB_postgres64 {
	...
	*/
	function MetaForeignKeys($table, $owner=false, $upper=false)
	{
	  ...
	  while (!$rs->EOF) {
	    if ($upper) {
	      $a[strtoupper($rs->Fields('lookup_table'))][] = strtoupper(strtr($rs->Fields('dep_field').'='.$rs->Fields('lookup_field'), '"', ''));
	    } else {
	      $a[$rs->Fields('lookup_table')][] = strtr($rs->Fields('dep_field').'='.$rs->Fields('lookup_field'), '"','');
	    }
	    adodb_movenext($rs);
	  }
