...
$TTXSetup::VERSION='3.01';
BEGIN {
  $TTXSetup::REVISION = '$Revision: 769 $';
...
my %colors = (
  'helpdesk-filter' => '#C6D7CF',
  'helpdesk-table-border' => '#FFFFFF',
  'helpdesk-table-header' => '#CCCCCC',
  'helpdesk-table-erow' => '#F0F0E1',
  'helpdesk-table-orow' => '#E8E8CF',
  'helpdesk-table-crow' => '#FFFFD9',
  'helpdesk-table-pending' => 'red',
  'helpdesk-table-open' => 'brown',
  'helpdesk-table-responded' => '#22AA55',
  'helpdesk-table-solved' => 'grey',  
  'helpdesk-table-onhold' => 'black',  
  'newticket-background' => '#CFDCE8',
  'ticket-background' => "#C6D7CF",
  'ticket-editor' => '#CFDCE8',
  'ticket-omsg' => '#E8E8CF',
  'ticket-imsg' => '#E8E8E8',
  'ticket-cmsg' => '#E8DCCF',
  'ticket-fomsg' => 'black',
...
td.trow  {
  ...
  border-left-width:1px;
  border-top-width:0;
  border-color: (%CONFIG_COLOR-HELPDESK-TABLE-BORDER%);
  background-color: (%CONFIG_COLOR-HELPDESK-TABLE-HEADER%);
  text-align: center;
  font-size : 11px;
  font-weight: 700;
}
...
td.omsg  {
  background-color: (%CONFIG_COLOR-TICKET-OMSG%);
  padding-right: 3pt;
  padding-left: 3pt;
  padding-top: 2pt;
  padding-bottom: 2pt;
  ...
  border-style:solid;
  border-color: #FFF;
  color: (%CONFIG_COLOR-TICKET-FOMSG%);
}
td.imsg  {
  background-color: (%CONFIG_COLOR-TICKET-IMSG%);
  padding-right: 3pt;
  padding-left: 3pt;
  padding-top: 2pt;
  padding-bottom: 2pt;
  ...
  border-style:solid;
  border-color: #FFF;
  color: (%CONFIG_COLOR-TICKET-FIMSG%);
}
td.cmsg  {
  background-color: (%CONFIG_COLOR-TICKET-CMSG%);
  padding-right: 3pt;
  padding-left: 3pt;
  padding-top: 2pt;
  padding-bottom: 2pt;
  ...
  border-style:solid;
  border-color: #FFF;
  color: (%CONFIG_COLOR-TICKET-FCMSG%);
}
</style>
...
<input type=hidden name=fltrsubj value="(%INPUT_fltrsubj%)">
        ...
        <tr>
          <td colspan=3 align=right class=sm>
            <a target=_blank href="(%ENV_SCRIPT_NAME%)?cmd=annotate&sid=(%INPUT_sid%)&key=(%INPUT_key%)&style=(%INPUT_style%)">[%Annotate%]</a>,&nbsp;&nbsp;
            <a target=_blank href="(%ENV_SCRIPT_NAME%)?cmd=print&sid=(%INPUT_sid%)&key=(%INPUT_key%)&style=(%INPUT_style%)">[%Print%]</a>&nbsp;&nbsp;
          </td>
        </tr>
...
<input type=hidden name=do value=1>
		...
		<tr>
				<td align=right><b>[%(%GROUPSELLBL%)%]</b></td>
				<td align=left>(%GROUPSEL%)</td>
		</tr>
		(%CUSTOMFIELDS%)
    <tr>
		    <td colspan=2>
...
sub setup4 {
  ...
  my ($mailer, $smtp, $home, $company, $email, $imgurl, $emailfix, $smtptrace,
      $smtplogin, $smtppwd, $hideold, $useaccesscode, $nomsgcfrm, $notikcfrm,
      $forcecharset, $sfcustomer, $sfoper, $stickycf, $rememberme, $useopersel,
      $emaillist, $askforname, $tracktime, $captchamode);
  if (!$query->param('do')) {
    ...
    $tracktime = $query->param('tracktime') || $cfg->get('tracktime');
    $emaillist = $query->param('emaillist') || $cfg->get('emaillist');
    $askforname = $query->param('askforname') || $cfg->get('askforname');
    $captchamode = $query->param('captchamode') || $cfg->get('captchamode');
    if ($cfg->get('blockautoresponder')) {
      $notikcfrm = 1;
      $nomsgcfrm = 1;
    }
    foreach my $color (keys %colors) {
      if ($cfg->get("color-$color") ne undef) {
        my $val = $cfg->get("color-$color");
        $val =~ s/\s//g;
        if (length($val) > 0) {
          $colors{$color} = $val;
        }
    ...
    $tracktime = $query->param('tracktime');
    $emaillist = $query->param('emaillist');
    $askforname = $query->param('askforname');
    $captchamode = $query->param('captchamode');
    if (!$query->param('resetcolors')) {
      foreach my $color (keys %colors) {
        my $val = $query->param("color-$color");
        $val =~ s/\s//g;
        $colors{$color} = $val;
      }
    }
  ...
  my $askfornamechecked = ' checked' if $askforname && ($emaillist ne undef);
  ...
  if ($query->param('do')) {
    ...
    $cfg->set('captchamode', $captchamode);
    $cfg->set('forcecharset', $forcecharset);
    $cfg->set('sendfiles.customer', $sfcustomer ? 1:0);
    $cfg->set('sendfiles.operator', $sfoper ? 1:0);
    if ($emaillist ne undef) {
      $cfg->set('askforname', $askforname ? 1:0);
    } else {
      $cfg->set('askforname', 0);
    }
    $cfg->set('emaillist', $emaillist);
    $cfg->set('tracktime', $tracktime);
    $cfg->set('blockautoresponder', '') if $cfg->get('blockautoresponder');
    if ($layout) {
      TTXLayout::set($cfg, $query);
    }
    foreach my $color (keys %colors) {
      $cfg->set("color-$color", $colors{$color});
    }
...
<td align=left>no</td></tr>
<tr><td colspan=4 align=left><br><center><b>Custom Fields</b></center><br><span class=sm><br><b>Custom fields.</b>
...
<a href="$ENV{SCRIPT_NAME}?cmd=export&pwd=$pwd">Export</a> |
<a href="$ENV{SCRIPT_NAME}?cmd=backup&pwd=$pwd">Backup/Restore</a>
</b></span>
<br>
...
EOT
  eval 'use GD';
  if ($@ eq undef) {
    my $checkit = ' checked' if $cfg->get('captchamode') =~ /alt/i;
    print <<EOT;
<tr><td colspan=2 align=left><span class=sm>
<b>Note:</b> The access code feature requires GD library. The library is not
a part of Trouble Ticket Express package. We detected the library installed on
your server. However it is impossible to check if the library was properly
configured. If you do not see the captcha image on your contact form after
enabling the above feature, please instruct the program to avoid using the GD
library (see the checkbox below). Doing this will activate embedded graphics
engine.
</span></td></tr>
<tr><td colspan=2 align=center>&nbsp;</td></tr>
<tr><td align=right><b>Do not use GD library</b></td><td align=left>&nbsp;&nbsp;
<input type=checkbox name=captchamode value=alt$checkit></td></tr>
<tr><td colspan=2 align=center>&nbsp;</td></tr>
EOT
  }
  print <<EOT;
...
<tr><td align=right><b>Carbon Copy field</b></td><td align=left>
...
EOT
  ...
  foreach my $o (@cflist) {
    ...
    print ' selected' if $emaillist eq $id;
    print ">$o</option>\n";
  }
  print <<EOT;
</select></td><td align=left>&nbsp;&nbsp;&nbsp;</td><td align=left><span class=sm>Select custom field that will hold a coma separated list of email addresses.
Each customer side email message will be copied to the list. Please use Custom Fields section below to manage custom fields.</span></td></tr></table></td></tr>
<tr><td colspan=2 align=center>&nbsp;</td></tr>
<tr><td align=right><b>Ask for name</b></td><td align=left>
<table cellspacing=0 cellpadding=0><tr>
<td align=left>&nbsp;&nbsp;</td>
<td align=left>
<input type=checkbox name=askforname value=1$askfornamechecked><td align=left>&nbsp;&nbsp;&nbsp;</td>
<td align=left><span class=sm>If the Carbon Copy option is active, multiple customers may post followup messages
to a ticket. Checking this box ensures that each customer will provide his or her name when posting a message.</span></td></tr>
</table></td></tr>
<tr><td colspan=2 align=center>&nbsp;</td></tr>
<tr><td align=right><b>Time tracking field</b></td><td align=left>
<table cellspacing=0 cellpadding=0><tr>
<td align=left>&nbsp;&nbsp;</td>
<td align=left>
<select name="tracktime">
<option value="">-- none --</option>
EOT
  @cflist = grep(/^x/, $cfg->vars());
  foreach my $o (@cflist) {
    my $id = $cfg->get($o);
    $id =~ s/\s//g;
    next if $id !~ /^\d\d?$/;
    $o =~ s/^x//;
    print "<option value=\"$id\"";
    print ' selected' if $tracktime eq $id;
    print ">$o</option>\n";
  }
  print <<EOT;
</select></td><td align=left>&nbsp;&nbsp;&nbsp;</td><td align=left><span class=sm>Select custom field that will hold a
total work time your operators spent for answering a ticket.
Please use Custom Fields section below to manage custom fields.</span></td></tr></table></td></tr>
<tr><td colspan=2 align=center>&nbsp;</td></tr>
<tr height=1><td colspan=2 align=center height=1 bgcolor="#9b9b9b"><img
src="$imgurl/dot.gif" height=1></td></tr>
<tr>
...
<tr>
<td colspan=3><br><center><b>HTML Settings</b></center>
<br>
</td>
</tr>
<tr>
...
<td colspan=2 align=left>
<input type=checkbox value=1 name=resetcolors>
(restore default colors)
</td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;filter</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-filter" value="$colors{'helpdesk-filter'}"></td>
<td bgcolor="$colors{'helpdesk-filter'}"><img src="$imgurl/dot.gif" height=1 width=80></td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;border</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-border" value="$colors{'helpdesk-table-border'}"></td>
<td bgcolor="$colors{'helpdesk-table-border'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;header</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-header" value="$colors{'helpdesk-table-header'}"></td>
<td bgcolor="$colors{'helpdesk-table-header'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;even row</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-erow" value="$colors{'helpdesk-table-erow'}"></td>
<td bgcolor="$colors{'helpdesk-table-erow'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;odd row</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-orow" value="$colors{'helpdesk-table-orow'}"></td>
<td bgcolor="$colors{'helpdesk-table-orow'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;current row</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-crow" value="$colors{'helpdesk-table-crow'}"></td>
<td bgcolor="$colors{'helpdesk-table-crow'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;pending</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-pending" value="$colors{'helpdesk-table-pending'}"></td>
<td bgcolor="$colors{'helpdesk-table-erow'}"><font color="$colors{'helpdesk-table-pending'}">&nbsp;Pending</font></td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;open</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-open" value="$colors{'helpdesk-table-open'}"></td>
<td bgcolor="$colors{'helpdesk-table-erow'}"><font color="$colors{'helpdesk-table-open'}">&nbsp;Open</font></td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;responded</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-responded" value="$colors{'helpdesk-table-responded'}"></td>
<td bgcolor="$colors{'helpdesk-table-erow'}"><font color="$colors{'helpdesk-table-responded'}">&nbsp;Responded</font></td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;solved</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-solved" value="$colors{'helpdesk-table-solved'}"></td>
<td bgcolor="$colors{'helpdesk-table-erow'}"><font color="$colors{'helpdesk-table-solved'}">&nbsp;Solved</font></td>
</tr>
<tr>
<td align=right><b>Ticket browser&nbsp;:&nbsp;on&nbsp;hold</b></td>
<td align=left><input type=text size=10 name="color-helpdesk-table-onhold" value="$colors{'helpdesk-table-onhold'}"></td>
<td bgcolor="$colors{'helpdesk-table-erow'}"><font color="$colors{'helpdesk-table-onhold'}">&nbsp;On&nbsp;hold</font></td>
</tr>
<tr>
<td align=right><b>Contact form&nbsp;:&nbsp;background</b></td>
<td align=left><input type=text size=10 name="color-newticket-background" value="$colors{'newticket-background'}"></td>
<td bgcolor="$colors{'newticket-background'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket&nbsp;:&nbsp;operator&nbsp;message&nbsp;background</b></td>
<td align=left><input type=text size=10 name="color-ticket-omsg" value="$colors{'ticket-omsg'}"></td>
<td bgcolor="$colors{'ticket-omsg'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket&nbsp;:&nbsp;operator&nbsp;message&nbsp;font</b></td>
<td align=left><input type=text size=10 name="color-ticket-fomsg" value="$colors{'ticket-fomsg'}"></td>
<td bgcolor="$colors{'ticket-omsg'}"><font color="$colors{'ticket-fomsg'}">&nbsp;Operator</font></td>
</tr>
<tr>
<td align=right><b>Ticket&nbsp;:&nbsp;internal&nbsp;message&nbsp;background</b></td>
<td align=left><input type=text size=10 name="color-ticket-imsg" value="$colors{'ticket-imsg'}"></td>
<td bgcolor="$colors{'ticket-imsg'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket&nbsp;:&nbsp;internal&nbsp;message&nbsp;font</b></td>
<td align=left><input type=text size=10 name="color-ticket-fimsg" value="$colors{'ticket-fimsg'}"></td>
<td bgcolor="$colors{'ticket-imsg'}"><font color="$colors{'ticket-fimsg'}">&nbsp;Internal</font></td>
</tr>
<tr>
<td align=right><b>Ticket&nbsp;:&nbsp;customer&nbsp;message&nbsp;background</b></td>
<td align=left><input type=text size=10 name="color-ticket-cmsg" value="$colors{'ticket-cmsg'}"></td>
<td bgcolor="$colors{'ticket-cmsg'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket&nbsp;:&nbsp;customer&nbsp;message&nbsp;font</b></td>
<td align=left><input type=text size=10 name="color-ticket-fcmsg" value="$colors{'ticket-fcmsg'}"></td>
<td bgcolor="$colors{'ticket-cmsg'}"><font color="$colors{'ticket-fcmsg'}">&nbsp;Customer</font></td>
</tr>
<tr>
<td align=right><b>Ticket&nbsp;:&nbsp;header</b></td>
<td align=left><input type=text size=10 name="color-ticket-background" value="$colors{'ticket-background'}"></td>
<td bgcolor="$colors{'ticket-background'}">&nbsp;</td>
</tr>
<tr>
<td align=right><b>Ticket&nbsp;:&nbsp;editor</b></td>
<td align=left><input type=text size=10 name="color-ticket-editor" value="$colors{'ticket-editor'}"></td>
<td bgcolor="$colors{'ticket-editor'}">&nbsp;</td>
</tr>
</table>
</td>
</tr>
      ...
      if ($user->{passwd} ne undef) {
        ...
        if ($addnew) {
          ...
          if (!$user->save()) {
            ...
            $newwrkhweek = $newwrkhsat = $newwrkhsun = "00:00-00:00";
            $newwrkhvac = undef;
          }
        }
      }
...
<br>
You may specify work hours for each operator to suppress new ticket email notifications. 00:00-00:00 means
always send notifications. Empty box (default) disables notifications for corresponding day.
Checking Vac (vacation) box disables notifications at any time.<br>
</td>
</tr>
</form>
</table>
...
sub grps {
  ...
  if ($query->param('do')) {
    ...
    foreach my $uid (@users) {
      ...
      foreach my $gid (@glist) {
        ...
        if ($mbrlist =~ /\b$uid\b/) {
          ...
          if ($query->param("mbr-$uid-$gid")) {
            $mbrlist .= ';' if $mbrlist ne undef && $mbrlist !~ /;$/;
            $mbrlist .= $uid;
            $cfg->set("mbr-$gid", $mbrlist);
            $dosave = 1;
          }
    ...
    foreach my $id ('usegrpsel', 'accessctrl', 'multigroup', 'additivetransfers', 'hideoperators') {
      if ($query->param($id) && !$cfg->get($id) && $grpmodule) {
        $cfg->set($id, 1);
        if ($id eq 'multigroup') {
          $cfg->set('startmultigroup', 1);
        }
        $dosave = 1;
      } elsif ($cfg->get($id) && (!$query->param($id) || !$grpmodule)) {
        $cfg->set($id, 0);
        if ($id eq 'multigroup') {
          $cfg->set('startmultigroup', 0);
        }
        $dosave = 1;
      }
    }
  ...
  my $checkhideoperators = ' checked' if $cfg->get('hideoperators');
  my $checkmultigroup = ' checked' if $cfg->get('multigroup');
  my $checkaddtrans = ' checked' if $cfg->get('additivetransfers');
...
<tr>
<td valign=top align=left><span class=lbl><nobr>Hide operators</nobr></span></td>
<td valign=top align=left><input type=checkbox name=hideoperators $disabled value=1$checkhideoperators></td>
<td valign=top align=left><span class=sm>If checked, the Trouble Ticket Express will show only departments the operator is a member of
and operators that are members of department(s) the operator is member of. This applies to ticket transfer, search filter and ticket
edit selectors available through operator interface.</span>
</td>
</tr>
<tr>
<td valign=top align=left><span class=lbl><nobr>Multiple Groups</nobr></span></td>
<td valign=top align=left><input type=checkbox name=multigroup $disabled value=1$checkmultigroup></td>
<td valign=top align=left><span class=sm>If checked, the Trouble Ticket Express will allow assigning multiple groups to a ticket.</span>
</td>
</tr>
<tr>
<td valign=top align=left><span class=lbl><nobr>Additive Transfers</nobr></span></td>
<td valign=top align=left><input type=checkbox name=additivetransfers $disabled value=1$checkaddtrans></td>
<td valign=top align=left><span class=sm>If checked, transferring a ticket to a group will result
in addition of the target froup to the ticket's group list. This option is ignored if Multiple Groups is off.</span>
</td>
</tr>
<tr>
...
EOT
  ...
  foreach my $login (sort @users) {
    ...
    next if $user eq undef || $user->{passwd} eq undef;
    print "<tr><td align=left><nobr>".TTXCommon::encodeit($user->{fname}).' '.TTXCommon::encodeit($user->{lname})." ($login)</nobr></td>\n";
    foreach my $id (sort @grplist) {
      my $grpname = $cfg->get($id);
      next if $grpname eq undef || $grpname eq '';
...
sub backup {
  ...
  $query = $_[1];
  eval "use TTXBackup";
  my $backupmodule = 1 if !$@;
  my $disabled = 'disabled=disabled' if !$backupmodule;
  my $pwd = $query->param('pwd');
  my $error = $query->param('error');
  if ($error ne undef) {
    print <<EOT;
<br>
<center><h3>Backup/Restore</h3>
<font color=red><b>$error</b></font>
<br><br>
<a href="$ENV{SCRIPT_NAME}?cmd=backup&pwd=$pwd">Continue</a>
</center>
EOT
    return;
  }
  my $wrkdir = $cfg->get('basedir'). '/backup';
  umask(0);
  if (! -e $wrkdir) {
    if (!mkdir($wrkdir, 0777)) {
      $error = "Can't create backup folder";
    }
  }
  if ($query->param('do')) {
    if (!$backupmodule) {
      $error = "Backup module is not available";
    } elsif($query->param('what') eq 'backup') {
      $error = TTXBackup::backup($cfg, $query->param('withfiles'));
    } elsif($query->param('what') eq 'restore') {
      $error = TTXBackup::restore($cfg, $query);
    }
  }
  print <<EOT;
<span class=sm><b><a href="$ENV{SCRIPT_NAME}?cmd=setup4&pwd=$pwd">System Setup</a> |
...
<tr>
<td valign=top align=left>
Backup Module allows saving complete Trouble Ticket Express data into a single file for archiving or transfers to new location.
The backup file includes the following information:
<br><ul>
<li>System configuration variables (excluding server related variables like file system paths, database settings, admin password)</li>
<li>Tickets</li>
<li>Log file (the file is required for reporting)</li>
<li>User database</li>
<li>Answer library</li>
<li>Inventory database</li>
<li>File attachments (optional)</li>
</ul>
The data from backup file may be restored on local or remote server. Please note that
<br>
<ul>
<li>Restoring data from backup file will delete (overwrite) all existing data, that is restoring from backup is not merge or sync operation.</li>
<li>While you may mix editions (for example do backup on Standard Edition with restore on MySQL Edition), you may not mix versions (e.g. backup on 2.25 with restore on 2.24).</li>
</ul>
For further info please visit <a target=_blank href="http://www.troubleticketexpress.com/backup.html">our web site</a>.
<br><br>
</td>
</tr>
EOT
...
<tr>
<td valign=top align=left>
<font color=red>No Backup Module found.</font> For ordering info please visit <a target=_blank href="http://www.troubleticketexpress.com/backup.html">our web site</a>.
<br><br>
</td>
</tr>
EOT
...
<tr>
<td valign=top align=left>
<center><b>1. Backup</b></center>
<br>
EOT
  my $wordnew = undef;
  if ($backupmodule && length($cfg->get('backup.last.file'))) {
    my $bkpfn = "$wrkdir/".$cfg->get('backup.last.file');
    if (-f $bkpfn) {
      my $timestamp;
      if (length($cfg->get('backup.last.time'))) {
        $timestamp = TTXCommon::tmtxt($cfg->get('backup.last.time'));
      }
      my $key = "K$$".int(1000000 + rand(9000000)).int(1000000 + rand(9000000));
      $cfg->set('backup.last.key', $key);
      $cfg->save();
      $wordnew = ' new';
      print <<EOT;
        <center><a href="$ENV{SCRIPT_NAME}?cmd=getbackup&key=$key&pwd=$pwd"><b>Download</b></a> existing backup file
        (timestamp: $timestamp)<br>
        <small>Local file name: $bkpfn</small><br><br></center>
EOT
    }
  }
  print <<EOT;
...
<input type=hidden name=do value=1>
<input type=hidden name=what value=backup>
<b>Generate</b> new backup file
<br><input type=checkbox name=withfiles value=1> Include file attachments (may result in large file)<br><br>
<input type=submit value="Generate$wordnew backup file">
</form>
<br><br>
<br>
<b>2. Restore</b>
...
<form action=$ENV{SCRIPT_NAME} method=post enctype="multipart/form-data" onsubmit="return onlyonce()">
<input type=hidden name=pwd value=$pwd>
<input type=hidden name=cmd value=backup>
<input type=hidden name=do value=1>
<input type=hidden name=what value=restore>
<br>Upload file: <input type=file name=upload size=50>
<br>or use file on server: <input type=text size=50 name=local>
<br><input type=checkbox name=withfiles value=1> Include file attachments (will delete local files if any)<br><br>
<input type=submit value="Restore from backup file">
</form>
</center>
</td>
</tr>
...
sub export {
  ...
  eval "use TTXExport";
  my $expmodule = 1 if !$@;
  my $disabled = 'disabled=disabled' if !$expmodule;
  my $pwd = $query->param('pwd');
  my $error = $query->param('error');
  if ($error ne undef) {
    print <<EOT;
<br>
<center><h3>Data Export</h3>
<font color=red><b>$error</b></font>
<br><br>
<a href="$ENV{SCRIPT_NAME}?cmd=export&pwd=$pwd">Continue</a>
</center>
EOT
    return;
  }
  my $wrkdir = $cfg->get('basedir'). '/export';
  umask(0);
  if (! -e $wrkdir) {
    if (!mkdir($wrkdir, 0777)) {
      $error = "Can't create export folder";
    }
  }
  if ($query->param('do')) {
    if (!$expmodule) {
      $error = "Export module is not available";
    } else {
      $error = TTXExport::export($cfg, $query);
    }
  }
  print <<EOT;
<span class=sm><b><a href="$ENV{SCRIPT_NAME}?cmd=setup4&pwd=$pwd">System Setup</a> |
...
The file includes all ticket fields. The file does not include messages or file attachments.
<br><br>
</td>
</tr>
EOT
...
<tr>
<td valign=top align=left>
<font color=red>No Export Module found.</font> For ordering info please visit <a target=_blank href="http://www.troubleticketexpress.com/export.html">our web site</a>.
<br><br>
</td>
</tr>
EOT
...
EOT
  my $wordnew = undef;
  if ($expmodule && length($cfg->get('export.last.file'))) {
    my $expfn = "$wrkdir/".$cfg->get('export.last.file');
    if (-f $expfn) {
      my $timestamp;
      if (length($cfg->get('export.last.time'))) {
        $timestamp = TTXCommon::tmtxt($cfg->get('export.last.time'));
      }
      my $key = "K$$".int(1000000 + rand(9000000)).int(1000000 + rand(9000000));
      $cfg->set('export.last.key', $key);
      $cfg->save();
      $wordnew = ' new';
      print <<EOT;
        <center><a href="$ENV{SCRIPT_NAME}?cmd=getexport&key=$key&pwd=$pwd">Download</a>
        (file timestamp: $timestamp)<br>
        <small>Local file name: $expfn</small><br><br></center>
EOT
    }
  }
  print <<EOT;
...
<input type=hidden name=do value=1>
<br>
<input type=submit value="Generate$wordnew data file">
</form>
<br>
</center>
</td>
...
sub build {
  ...
  my $saveit;
  foreach my $color (keys %colors) {
    if ($cfg->get("color-$color") ne $colors{$color}) {
      $saveit = 1;
      $cfg->set("color-$color", $colors{$color});
    }
  }
...
EOT
  ...
  eval "use Image::Size";
  if ($@ ne undef) {
    print "No Image::Size module found.<br>\n";
  } else {
    eval "$Image::Size::VERSION;";
    if ($@ eq undef) { print 'Image:'.":Size module version $Image::Size::VERSION found<br>\n"; }
  }
  eval "use Time::Local";
  if ($@ ne undef) {
...
my $_grpenabled = undef;

sub gsel {
  my ($id, $igroup, $query) = @_;
  my $sel;
  if ($_grpenabled eq undef) {
    eval 'use TTXGroups';
    if ($@ eq undef) {
      $_grpenabled = 1;
    } else {
  ...
  return undef if !$_grpenabled;
  if ($query->param('do') eq 'items') {
    $sel =  $query->param($id);
  } elsif ($igroup ne undef) {
    $sel = $igroup;
  }
  my $lbl = TTXCommon::decodeit($cfg->get('grpsellbl')) || 'Group';
  my $buff = "<b><small>$lbl</small></b><br>\n<select name=$id>\n<option></option>\n";
  my @grplist = grep(/^group\d+$/, $cfg->vars());
  foreach my $gid (sort {uc(TTXCommon::decodeit($cfg->get($a))) cmp uc(TTXCommon::decodeit($cfg->get($b)))} @grplist) {
    my $grpname = TTXCommon::decodeit($cfg->get($gid));
    next if $grpname eq undef || $grpname eq '';
    $grpname =~ s/</&quot;/g;
    $buff .= "<option value=$gid";
    $buff .= ' selected' if $sel eq $gid;
    $buff .= ">$grpname</option>\n";
  }
  $buff .= "</select>\n";
  return $buff;
...
#
  ...
  my $newemail = TTXCommon::cleanit($query, 'newemail');
  my $newgroup = TTXCommon::cleanit($query, 'newgroup');
  if ($query->param('do') eq 'items') {
    my $newid;
    TTXInventory::lockdb();
    if ($newtitle ne undef) {
      ...
      $item{email} = TTXCommon::decodeit($newemail);
      $item{group} = TTXCommon::decodeit($newgroup);
      if (length($item{email}) > 0 && $item{email} !~ /^[0-9A-Za-z'._+-]+@[0-9A-Za-z_-]+\.[0-9A-Za-z._-]+$/) {
        print <<EOT;
<center>
<br><b><font color=red>Invalid email address: $item{email}</red></b>
</center>
EOT
      } else {
        ...
        if ($newid eq undef) {
          ...
          print <<EOT;
<center>
<br><b><font color=red>Error: $err</red></b>
</center>
EOT
        } else {
          $query->param("group$newid", $query->param('newgroup'));
          $query->param('newgroup', '');
          $newtitle = $newimg = $newnotes = $newemail = $newgroup = undef;
        }
      }
    }
#
...
#
    ...
    foreach my $id (@ilist) {
      ...
      if (length($item->{title}) < 1) {
        push @errors, "Missing title for item $id";
      }
      $item->{img} = TTXCommon::cleanit($query, "img$id", 1);
      $item->{notes} = TTXCommon::cleanit($query, "notes$id", 1);
      $item->{email} = TTXCommon::cleanit($query, "email$id", 1);
      if (length($item->{email}) > 0 && $item->{email} !~ /^[0-9A-Za-z'._+-]+@[0-9A-Za-z_-]+\.[0-9A-Za-z._-]+$/) {
        push @errors, 'Invalid email address: '.$item->{email}." for $id:".$item->{title};
      }
      $item->{group} = TTXCommon::cleanit($query, "group$id", 1);
    }
    if (@errors > 0) {
      my $err = join('<br>\n', @errors);
      print <<EOT;
<center>
<br><b><font color=red>$err</red></b>
</center>
EOT
    } else {
      TTXInventory::savedb();
...
EOT
  ...
  foreach my $id (@ilist) {
    ...
    my $email = TTXCommon::encodeit($item->{email});
    $email =~ s/"/&quot;/g;
    $notes =~ s/</&lt;/g;
    my $groupsel = gsel("group$id", $item->{group}, $query);
    print <<EOT;
<tr>
<td align=center>
...
<input type=text name=email$id size=25 value="$email">
</td>
<td>&nbsp;</td>
<td align=left>
$groupsel
...
EOT
  ...
  my $newemail_quot = $newemail;
  $newemail_quot =~ s/"/%quot;/g;
  my $groupsel = gsel('newgroup', undef, $query);
print <<EOT;
<tr>
...
<td align=left>
<b><small>Email</small></b>
<input type=text name=newemail size=25 value="$newemail_quot">
</td>
<td>&nbsp;</td>
<td align=left>
$groupsel
