    ...
    */
    function draw() {
        ...
        echo "\n\n<!-- START NAVIGATION -->\n";
         //Selección de Titulo
		$nombre=" ";
		$con = mysql_connect('localhost', 'miguel', 'm1g23l');
		if (!$con)
 		 {
  		die('Fallo la conexión ' . mysql_error());
 		 }
		mysql_select_db("grupo", $con);
//echo "SELECT vorname FROM phpr_users WHERE ID=".$GLOBALS['user_ID'];
		$nombre = mysql_query("SELECT vorname FROM phpr_users WHERE ID=".$GLOBALS['user_ID']);
		$materia =mysql_query("SELECT name FROM phpr_gruppen WHERE ID=".$GLOBALS['user_group']);
//var_dump( $nombre);
while($row = mysql_fetch_array($nombre))
  {
  $dato= $row['vorname'];
  }
while($row = mysql_fetch_array($materia))
  {
  $dato2= $row['name'];
  }
		...
		echo '<div id="global-navigation"> <h3><b>Bienvenido:</b></h3></br><h3 style:"align=center;">'.$dato.'</h3></br><h3>Materia:</h3></br><h4>'.$dato2.'</h4></br>
	...
	//Aqui me quede
		$url="http://telecom.fi-b.unam.mx/group/lib/ImaGroup/".$dato2.".jpg";
	 	$existe=@fopen($url,'r');
		if ($existe){
			echo 'Existe el archivo';
		}else{
			echo'<img src="http://telecom.fi-b.unam.mx/group/lib/ImaGroup/default.jpg">';
		}
    ...
    */
    function get_group_box() {
        ...
        if (count($groups)>0){
            ...
            $out .=
            "\t"
            .'<b>Mis grupos</b><br/><form name="grsel" action="../index.php" method="post">'
            .' <input type="hidden" name="csrftoken" value="'.make_csrftoken().'" />'
            ."\n\t"
