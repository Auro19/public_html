...
{	
	...
	function addGroupForm()
	{
		...
		$LANG = new GKLang();
		// showing form
		require_once(JPATH_COMPONENT.DS.'configuration'.DS.'plugins'.DS.'plg_gk_news_image_1'.DS.'views'.DS.'add.group.php');
	}	
	...
	function editGroupForm()
	{
		...
		// showing form
		require_once(JPATH_COMPONENT.DS.'configuration'.DS.'plugins'.DS.'plg_gk_news_image_1'.DS.'views'.DS.'edit.group.php');
	}	
	// Editing group	
	function editGroup($apply = false){
		...
		require_once(JPATH_COMPONENT.DS.'configuration'.DS.'plugins'.DS.'plg_gk_news_image_1'.DS.'class.image.php');
		$img = new Image();
		$db =& JFactory::getDBO();
		$plugin	= JRequest::getCmd('plugin');
		$option	= JRequest::getCmd('option');
	...
	function addSlideForm(){
		...
		// showing form
		require_once(JPATH_COMPONENT.DS.'configuration'.DS.'plugins'.DS.'plg_gk_news_image_1'.DS.'views'.DS.'add.slide.php');
	}
	// adding slide
	function addSlide(){
		unset($LANG);
		...
		// uploading image
		$hash = $img->upload($mW, $mH, $sW, $sH, '#000000', $quality);
	...
	function editSlideForm(){
		...
		$db->setQuery( 'SELECT a.`id` AS `id` , a.`title` AS `art_title`, k.`title` AS `cat_name` FROM `#__content` AS `a` LEFT JOIN `#__categories` AS `k` ON a.`catid` = k.`id` ORDER BY k.`title` ASC;' );
		// showing form
		require_once(JPATH_COMPONENT.DS.'configuration'.DS.'plugins'.DS.'plg_gk_news_image_1'.DS.'views'.DS.'edit.slide.php');
	}	
	// Editing slide
	...
	function previewGroup()
	{
		...
		// showing form
		require_once(JPATH_COMPONENT.DS.'configuration'.DS.'plugins'.DS.'plg_gk_news_image_1'.DS.'views'.DS.'preview.group.php');	
	}
	// preview for slide
	function previewSlide()
	{
		...
		// showing form
		require_once(JPATH_COMPONENT.DS.'configuration'.DS.'plugins'.DS.'plg_gk_news_image_1'.DS.'views'.DS.'preview.slide.php');		
	}	
}
