...
$TTXMail::VERSION='3.01';
BEGIN {
  $TTXMail::REVISION = '$Revision: 769 $';
...
sub newsmtp {
  my ($smtphost, $debug) = @_;
  my $smtpobj = TTXData::get('SMTPOBJ');
  if ($smtpobj eq undef) {
    $debugon = $debug;
    $smtpobj = Net::SMTP->new($smtphost, Debug => $debug);
    $trysmtpauth = 1;
  } else {
    $trysmtpauth = 0;
  }
  TTXData::set('SMTPOBJ', $smtpobj);
  return $smtpobj;
}
# ========================================================================= quit
...
sub sendmail {
  ...
  if (!smtp()) {
    ...
    $from =~ s/^\s+//;
    $from =~ s/\s+$//;
    return 0 if !open(MAIL, "|$mailer -t -f $from");
    my $bcc = "Bcc: ".$_[0]->{bcc}."\n" if $_[0]->{bcc} ne undef;
    ...
    my $s = newsmtp($smtphost, $cfg->get('smtptrace') ? 1:0);
    if ($s ne undef) {
      if ($trysmtpauth) {
	$trysmtpauth = 0;
