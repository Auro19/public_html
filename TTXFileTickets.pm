...
$TTXFileTickets::VERSION='3.01';
BEGIN {
  $TTXFileTickets::REVISION = '$Revision: 769 $';
...
sub addticket {
  ...
  $tik->{updated} = $tik->{open} if $tik->{updated} eq undef;
  ...
  if ($filter ne undef) {
    foreach my $f (@{$filter}) {
      ...
      if ($c eq 'text') {
        ...
        if ($grps ne undef) {
          @list = grep {$tickets->{$_}->{oper} eq $oper || $tickets->{$_}->{grp} =~ /\b$grps\b/} @list;
        } else {
        ...
        $expr =~ s/^\^//; $expr =~ s/\$$//;
        if (!length($expr)) {
          @list = grep {!length($tickets->{$_}->{grp})} @list;
        } else {
          @list = grep {$tickets->{$_}->{grp} =~ /\b$expr\b/i} @list;
        }
      } else {
        $byid = 1 if $c eq 'id';
